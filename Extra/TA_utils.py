...
# Staking Data NN Performance Functions
# ------------------------------------------------------------------------------
...
def tangency_weights(returns, cov_mat = 1):
    # Returns should have no date column. 
...
def gmv_weights(tot_returns):
    # No date column for tot_returns
...
def mv_portfolio(target_ret, tot_returns):
    # No date column for tot_returns
    # target_ret should be for the same period as the data given (if data is monthly, the target_ret should be target monthly return)
    # doesn't take into account a rfr
...
# To hit a target in sample mean, do the following:
# 1. Get the current mean returns using current wieghts: returns.mean() @ weights
# 2. Multiply the current weights by the target return: weights * target_return
# 3. Divide 2./1.: weights = (weights * target_return) / (returns.mean() @ weights)
